...
def cross(series_a, series_b, above = True, asint = True, offset = None,
          **kwargs):
    """series_a: pd.Series, series_b: pd.Series, above: bool = True, asint: bool = True, offset: int = None,
          **kwargs"""
    ...
    cross.name = "{}_{}_{}".format(series_a.name, 'XA' if above else 'XB', series_b.name)
...
def df_error_analysis(dfA, dfB, **kwargs):
    ...
    extra = pd.DataFrame([diff.var(), diff.mad(), diff.sem(), dfA.corr(dfB, method=corr_method)],
...
def signed_series(series, initial = None):
    """Returns a Signed Series with or without an initial value
    series: pd.Series, initial: int = None"""
