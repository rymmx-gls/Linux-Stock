...
def check_monthly_trade_date(ts_code, trade_date):
    """检查该日数据是否已经存在"""

    sql = '''select trade_date from `monthly_{}` where trade_date={}'''.format(ts_code, trade_date)
    try:
        cursor.execute(sql)
        if cursor.fetchall():
            return True
        ...
        outinfo("创建新股票表:monthly_{}".format(ts_code))
        sql = '''create table if not exists stock.`monthly_{}`
                (
                    ts_code text null,
...
def start():
    ...
    res = session.query(StockAttrModel).filter(StockAttrModel.item == 'monthly_update_stop_date').first()
    stop_date = res.value
    for date in date_list:
        if date > stop_date:
            trade_date = date
            df = pro.monthly(trade_date=trade_date)
            if not df.empty:
                for i in df.to_dict('records'):
                    if not check_monthly_trade_date(i['ts_code'], i['trade_date']):
                        tbname = "monthly_" + i['ts_code']
...
if __name__ == '__main__':
    init_logger("task_monthly_update")
    outinfo("start task_monthly_update")
    start()
    outinfo("end task_monthly_update")