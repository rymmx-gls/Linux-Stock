...
def check_weekly_trade_date(ts_code, trade_date):
    """检查该日数据是否已经存在"""

    sql = '''select trade_date from `weekly_{}` where trade_date={}'''.format(ts_code, trade_date)
    try:
        cursor.execute(sql)
        if cursor.fetchall():
            return True
        ...
        outinfo("创建新股票表:weekly_{}".format(ts_code))
        sql = '''create table if not exists stock.`weekly_{}`
                (
                    ts_code text null,
...
def start():
    ...
    res = session.query(StockAttrModel).filter(StockAttrModel.item == 'weekly_update_stop_date').first()
    stop_date = res.value
        ...
        if date > stop_date:
            ...
            if not df.empty:
                for i in df.to_dict('records'):
                    if not check_weekly_trade_date(i['ts_code'], i['trade_date']):
                        tbname = "weekly_" + i['ts_code']
...
if __name__ == '__main__':
    ...
    outinfo("start task_weekly_update")
    start()
    outinfo("end task_weekly_update")